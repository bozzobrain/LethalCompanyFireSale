		...
		public void PlaceStorageObjectOnShelve(List<GrabbableObject> objectsOfType)
		{
			...
			for (int i = 0; i < objectsOfType.Count; i++)
			{
				...
				if (!IsPositionWithinCloset(placementLocation))
				{
					...
					FireSale.Log($"Fixed object location out of closet {obj.name}");
				}
				FireSale.Log($"Placing {obj.name} [{i + 1} of {objectsOfType.Count}] on shelve {shelveToPlaceOn} at {placementLocation.x},{placementLocation.y},{placementLocation.z}");
